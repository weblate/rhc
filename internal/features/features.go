...
// isFeatureEnabledInPreferenceFile checks if the feature is enabled in the preference file.
func isFeatureEnabledInPreferenceFile(featureID string) (bool, error) {
	knownFeatures := MapKnownFeatureIds()
	var feature *RhcFeature
	var exists bool
	if feature, exists = knownFeatures[featureID]; !exists {
		return false, fmt.Errorf("unknown feature ID: '%s'", featureID)
	}
	if enabled := feature.IsEnabledInConf(); enabled != nil {
		return *enabled, nil
	}
	return false, nil
}
